...
export default function FailedPage() {
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <h1 className="text-2xl sm:text-3xl font-bold text-white">Failed Batches</h1>
        ...
        <CardContent>
          {allFailedBatches.length === 0 ? (
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4 rounded-lg border border-gray-700 bg-gray-800/50 p-3">
                ...
                >
                  ...
                  {isBulkRetrying
                    ? 'Retrying...'
                    : `Retry${selectedIds.size > 0 ? ` (${selectedIds.size})` : ' Selected'}`}
                </Button>
              </div>
                    ...
                    >
                      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:gap-4">
                              ...
                              <div className="mt-2">
                                {batch.jobs
                                  ...
                                  .map((job) => (
                                    <p
                                      key={job.id}
                                      className="text-xs sm:text-sm text-red-400 flex items-start gap-1"
                                    >
                            ...
                            <div className="mt-3">
                              ...
                              <div className="flex flex-wrap gap-2 sm:gap-4 mt-1 text-xs">
                                <span className="flex items-center gap-1 text-green-400">
                                  ...
                                  <span className="hidden sm:inline">{progress.completed} completed</span>
                                  <span className="sm:hidden">{progress.completed}</span>
                                </span>
                                <span className="flex items-center gap-1 text-red-400">
                                  <span className="h-2 w-2 rounded-full bg-red-500" />
                                  <span className="hidden sm:inline">{progress.failed} failed</span>
                                  <span className="sm:hidden">{progress.failed}</span>
                                </span>
                                {progress.pending > 0 && (
                                  <span className="flex items-center gap-1 text-gray-400">
                                    <span className="h-2 w-2 rounded-full bg-gray-600" />
                                    <span className="hidden sm:inline">{progress.pending} pending</span>
                                    <span className="sm:hidden">{progress.pending}</span>
                                  </span>
                                )}
                              </div>
                            </div>
                        ...
                        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:flex-shrink-0">
                          <Link href={`/dashboard/batches/${batch.id}`} className="w-full sm:w-auto">
